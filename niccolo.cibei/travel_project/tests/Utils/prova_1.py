...
class TravelGraph:
    ...
    def world_tour(self, start_city_name):
        # Find the corresponding ID for the starting city
        start_id = self.df.loc[self.df['city_ascii'] == start_city_name, 'id'].values[0]

        # Utilizza l'algoritmo di Dijkstra per trovare il percorso più breve da una città di partenza a tutte le altre
        shortest_paths = nx.single_source_dijkstra_path_length(self.G, start_id, weight='weight')

        # Trova il percorso più breve tornando alla città di partenza
        shortest_path = nx.shortest_path(self.G, source=start_id, target=start_id, weight='weight')
